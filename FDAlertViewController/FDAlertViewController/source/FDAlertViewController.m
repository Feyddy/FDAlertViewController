...
@interface FDHighLightButton : UIButton
...
@implementation FDHighLightButton
...
@interface FDAlertAction ()

@property (copy, nonatomic) void(^actionHandler)(FDAlertAction *action);

@end

@implementation FDAlertAction

+ (instancetype)actionWithTitle:(NSString *)title handler:(void (^)(FDAlertAction *action))handler {
    FDAlertAction *instance = [FDAlertAction new];
...
@interface FDAlertViewController ()
{
    UIView *_shadowView;
    UIView *_contentView;
...
@implementation FDAlertViewController

+ (instancetype)alertControllerWithTitle:(NSString *)title message:(NSString *)message {
    
    FDAlertViewController *instance = [FDAlertViewController new];
...
- (void)creatAllButtons {
    ...
    for (int i=0; i<self.actions.count; i++) {
        ...
        FDHighLightButton *btn = [FDHighLightButton new];
        btn.tag = 10+i;
...
- (void)didClickButton:(UIButton *)sender {
    FDAlertAction *action = self.actions[sender.tag-10];
